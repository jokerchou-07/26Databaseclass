...
| **Task 4** — Neo4j graph design & seeding (`seed_neo4j.py`, `seed.cypher`) | 周怡辰| 林誼婷 梁易軒| 由Primary Owner製作、修改可完整運行程式，Supporting Member負責測試、小幅度修改|
| **Task 5** — Neo4j query functions (`graph/queries.py`) | 林誼婷 | 周怡辰 梁易軒| 由Primary Owner製作、修改可完整運行程式，Supporting Member負責測試、小幅度修改|
